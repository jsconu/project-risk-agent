...
class JiraConnector(ProjectConnector):
    ...
    For a personal/local integration this connector supports email + API token.
    A hosted multi-user application should use Atlassian OAuth 2.0 instead.
    """
    ...
    def get_projects(self) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            projects.extend(
                {"id": str(item["id"]), "key": item.get("key"), "name": item.get("name")}
                for item in values
            )
    ...
    def get_signals(self, project_id: str) -> list[ProjectSignal]:
        ...
        while True:
            params: dict[str, str | int] = {
                ...
                "maxResults": 100,
                "fields": "summary,status,description,updated,assignee,priority",
            }
            if next_page_token:
    ...
    def normalize(self, source_data: Any) -> ProjectSignal:
        ...
        timestamp = _parse_timestamp(fields.get("updated"))
        return ProjectSignal(
            id=f"jira-{issue_key}",
            source="jira",
            source_type="issue",
            timestamp=timestamp,
            project_id=(fields.get("project") or {}).get("key"),
            author=((fields.get("assignee") or {}).get("displayName")),
            content=content,
            metadata={"issue_key": issue_key, "status": status, "priority": priority},
            provenance={"connector": self.name, "resource": f"/browse/{issue_key}"},
        )
